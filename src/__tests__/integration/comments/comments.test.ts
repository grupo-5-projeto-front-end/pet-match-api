...
import { mockedComment, mockedSegundLogin, mockedSegundUser, mockedUser, mockedUserLogin } from "../../mocks";
...
describe("/comments", () => {
    ...
    test("POST /comments/id - Should be able to create a comment", async () => {
        ...
        const response = await request(app).post(`/comments/${user.body[1].id}`).set("Authorization", `Bearer ${loginRes.body.token}`).send(mockedComment);
    ...
    test("POST /comments/id - Should NOT be able to create a comment without authentication", async () => {
      ...
      mockedComment.userId = user.body[0].id
        ...
        const response = await request(app).post(`/comments/${user.body[1].id}`).send(mockedComment)       
    ...
    test("PATCH /comments - Should be able to update a comment", async () => {
        ...
        const response = await request(app).patch(`/comments/${commentId}`).set("Authorization", `Bearer ${loginRes.body.token}`).send({comment: "este comentário foi atualizado"});
        expect(response.status).toBe(201);
        expect(response.body).toHaveProperty("id");
        expect(response.body).toHaveProperty("userId");
        expect(response.body).toHaveProperty("comment");
    ...
    test("PATCH /comments - Should NOT be able to update a comment without authentication", async () => {
      ...
      const response = await request(app).patch(`/comments/${commentId}`)
  ...
  test("DELETE /comments - Should be able to delete a comment", async () => {
    ...
    const response = await request(app).delete(`/comments/${commentId}`).set("Authorization", `Bearer ${loginRes.body.token}`)
